...
graphiti_agent = Agent(
    get_model(),
    system_prompt="""You are a helpful assistant with access to a knowledge graph filled with temporal data about Sustainable America.
...
async def search_graphiti(ctx: RunContext[GraphitiDependencies], query: str) -> List[GraphitiSearchResult]:
    ...
    print(f"[Graphiti Search] Query: {query}")

    try:
        # Perform the search
        results = await graphiti.search(query)
        print(f"[Graphiti Search] Raw results count: {len(results)}")
        ...
        print(
            f"[Graphiti Search] Formatted results count: {len(formatted_results)}")
        return formatted_results
    except Exception as e:
        # Log the error but don't close the connection since it's managed by the dependency
        print(f"[Graphiti Search] Error searching Graphiti: {str(e)}")
        raise
